...
namespace WebChat.DataAccess.MsSql.Configurations
{
    public class UserPhotoCommentLikeConfiguration : IEntityTypeConfiguration<UserPhotoCommentLike>
    {
        public void Configure(EntityTypeBuilder<UserPhotoCommentLike> builder)
        {
            ...
            builder.HasOne(prop => prop.UserProfile).WithMany(prop => prop.UserPhotoCommentLikes).HasForeignKey(prop => prop.UserProfileId);
            builder.HasOne(prop => prop.UserPhotoComment).WithMany(prop => prop.Likes).HasForeignKey(prop => prop.UserPhotoCommentId).OnDelete(DeleteBehavior.Restrict);
        }
    }
}
