...
namespace WebChat.Application.Validators
{
    public class UpdateAccountCommandValidator : AbstractValidator<UpdateAccountCommand>
    {
        public UpdateAccountCommandValidator()
        {
            ...
            RuleFor(prop => prop.CurrentPassword).Length(8, 20).WithMessage($"Minimum password length {8} Maximum length ${20} characters");
            RuleFor(prop => prop.NewPassword).Length(8, 20).WithMessage($"Minimum password length {8} Maximum length ${20} characters");
        }
    }
}
